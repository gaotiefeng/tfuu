 ...
 * <p>
 * The implementation adheres to the <a
 * href="http://rfc-editor.org/rfc/rfc7493.txt">RFC-7493</a> to support Temporal
...
public class JsonObject implements Iterable<Map.Entry<String, Object>>,
    ...
    public void putAll(JsonObject jsonObject) {
        this.map.putAll(jsonObject.getMap());
    }
    ...
    public JsonObject appendAll(Map<String, Object> map) {
        putAll(map);
        return this;
    }

    public void pulAll(Map<String, Object> map) {
        this.map.putAll(map);
    }
    ...
    public static JsonObject mapFrom(Object obj) {
        return new JsonObject((Map<String, Object>) Json.mapper.convertValue(
                obj, Map.class));
    }
    ...
    public Instant getInstant(String key) {
        Objects.requireNonNull(key);
        String encoded = (String) map.get(key);
        return encoded == null ? null : Instant
                .from(ISO_INSTANT.parse(encoded));
    }
    ...
    public String getString(String key, String def) {
        Objects.requireNonNull(key);
        CharSequence cs = (CharSequence) map.get(key);
        return cs != null || map.containsKey(key) ? cs == null ? null : cs
                .toString() : def;
    }
    ...
    public byte[] getBinary(String key, byte[] def) {
        Objects.requireNonNull(key);
        Object val = map.get(key);
        return val != null || map.containsKey(key) ? (val == null ? null
                : Base64.getDecoder().decode((String) val)) : def;
    }
    ...
    public Instant getInstant(String key, Instant def) {
        ...
        Object val = map.get(key);
        return val != null || map.containsKey(key) ? (val == null ? null
                : Instant.from(ISO_INSTANT.parse((String) val))) : def;
    }
     ...
     * String using the {@link Enum#name} method and the value put as a String.
     *
     * @param key
     *            the key
     * @param value
    ...
    public JsonObject put(String key, byte[] value) {
        Objects.requireNonNull(key);
        map.put(key,
                value == null ? null : Base64.getEncoder()
    ...
    public JsonObject put(String key, Object value) {
        ...
        if (value instanceof JsonObject)
            value = ((JsonObject) value).getMap(); // json 转换出现，反序列化 兼容问题.
        ...
        for (Map.Entry<String, Object> e : other.map.entrySet()) {
            map.merge(e.getKey(), e.getValue(), (oldVal, newVal) -> {
                ...
                if (oldVal instanceof JsonObject
                        && newVal instanceof JsonObject) {
                    return ((JsonObject) oldVal).mergeIn((JsonObject) newVal,
                            depth - 1);
                }
                return newVal;
            });
    ...
    private class Iter implements Iterator<Map.Entry<String, Object>> {
        ...
        public Map.Entry<String, Object> next() {
            ...
            if (entry.getValue() instanceof Map) {
                return new Entry(entry.getKey(), new JsonObject(
                        (Map) entry.getValue()));
            } else if (entry.getValue() instanceof List) {
                return new Entry(entry.getKey(), new JsonArray(
                        (List) entry.getValue()));
            }
            return entry;
        }
